    ...
    class Indexer < Jekyll::Generator
      def initialize(config = {})
        ...
        if lunr_config.key?('excludes')
          Jekyll.logger.debug "Lunr:", "TEST: contains excludes key"
          if lunr_config['excludes'].is_a?(Hash)
            Jekyll.logger.debug "Lunr:", "TEST: is hash"
            @excludes.merge!(lunr_config['excludes'] || {})
          else
            Jekyll.logger.debug "Lunr:", "TEST: is array"
            @excludes['files'] = lunr_config['excludes']
          end
        end
        Jekyll.logger.debug "Lunr:", "TEST: #{@excludes}"
